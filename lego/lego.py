      ...
      if mqtt:
         ...
         def on_message(client, userdata, msg):
            ...
            if parts[3] == 'next_brick': # .../input/next_brick/...
               ...
               if k == 'action':
                  if parts[5] == 'rotate':
                     self.active_brick.length_vec = angleToBrickDirection(self.mc.player.getRotation())
                     self.active_brick.draw()
                  elif parts[5] == 'move':
                     self.active_brick.ask_position()
               elif k == 'block':
                  v = Block(*[int(x) for x in str(msg.payload).split(',')])
                  self.active_brick.block = v
                  self.active_brick.draw()
               elif k in ['width','length','height','has_studs'] and \
                     not self.active_brick.has_custom_shape:
                  # presume it's an attribute and attempt to set it
                  v = int(msg.payload)
                  try:
                     setattr(self.active_brick,k,v)
                     self.active_brick.rebuild_shape()
                     self.active_brick.draw()
                  except:
                     import traceback
                     print traceback.format_exc()
            ...
            if self.active_brick:
               ...
               if last_active_brick_data:
                  for k in self.next_brick: # active_brick has same attrs we care about
                     if getattr(self.active_brick,k) != last_active_brick_data.get(k):
                        changed = True
               else:
                  changed = True
